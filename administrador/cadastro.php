...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email) || empty($senha) || empty($confirmar_senha)) {
        ...
        $sql = "INSERT INTO cadastros (email,telefone, senha) VALUES ('$email','$telefone', '$senha')";

        if ($conn->query($sql) === TRUE) {
            $message = "<div class='alert alert-success'>Cadastro realizado com sucesso! <a href='indox.php'>Faça o login!</a></div>";
            $showForm = false;
        } else {
        ...
        <div class="form-container">
            <form action="cadastro.php" method="post" class="registration-form">
                ...
                <div class="input-group">
                    ...
                    <input type="tel" id="telefone" name="telefone" pattern="^\(\d{2}\)\d{4,5}-\d{4}$" autocomplete="telefone" required>
                </div>
